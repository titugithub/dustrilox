...
<div id="comments" class="blog-post-comment">
    ...
    $defaults = [
        ...
        ',
        'submit_button'    => '<div class="col-xl-12"><button class="tp-btn" type="submit">' . esc_html__( 'Post Comment', 'dustrilox' ) . ' </button></div>',
        /** This filter is documented in wp-includes/link-template.php */
        'must_log_in'        => '
            <p class="must-log-in">
            '.esc_html__('You must be','dustrilox').' <a href="'.esc_url(wp_login_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) )).'">'.esc_html__('logged in','dustrilox').'</a> '.esc_html__('to post a comment.','dustrilox').'
            </p>',
        /** This filter is documented in wp-includes/link-template.php */
        'logged_in_as'       => '
            <p class="logged-in-as">
            '.esc_html__('Logged in as','dustrilox').' <a href="'.esc_url(get_edit_user_link()).'">'.esc_html($user_identity).'</a>. <a href="'.esc_url(wp_logout_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) )).'" title="'.esc_attr__('Log out of this account','dustrilox').'">'.esc_html__('Log out?','dustrilox').'</a>
            </p>',
        'id_form'            => 'commentform',
        'id_submit'          => 'submit',
        'class_submit'       => 'tp-btn',
        'title_reply'        => esc_html__( 'Leave a Reply', 'dustrilox' ),
        'title_reply_to'     => esc_html__( 'Leave a Reply to %s', 'dustrilox' ),
        'cancel_reply_link'  => esc_html__( 'Cancel reply', 'dustrilox' ),
        'label_submit'       => esc_html__( 'Post Comment', 'dustrilox' ),
        'format'             => 'xhtml',
    ];
